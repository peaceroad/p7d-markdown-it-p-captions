...
- Branded runtime counters are partitioned by mark and by the callback's primitive `sequenceKey`; `null` selects the mark's unscoped sequence and `undefined` is a contract error.
